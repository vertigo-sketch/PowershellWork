...
do {
	...
	# Device Preparation "APv2"
	$DetectedInAPv2Mode=$false
	$DetectedOutFromAPv2Mode=$false
	$APv2ModeStartTime = $null
	$APv2ModeEndTime = $null
	...
	While ($i -lt $LogEntryList.Count) {
		...
		# DEBUG manually line by line
		#Write-Host "$($LogEntryList[$i].Message) - `$i=$($i)"
		#Pause
		...
		if((-not $NotInEspInSessionFound) -and (-not $DetectedInAPv2Mode)) {
		...
		# Device Preparation mode detected
		if($DetectedInAPv2Mode -eq $false) {
			# Works also
			#if($LogEntryList[$i].Message -match "\[APv2\] Device is in APv2 mode: True.") {

			if($LogEntryList[$i].Message -eq '[APv2] Device is in APv2 mode: True.') {

				$LogEntryList[$i].ProcessRunTime = "Device Preparation mode (APv2) detected"
				
				$DetectedInAPv2Mode = $True
				
				$APv2ModeStartTime = $LogEntryList[$i].DateTime
				
				RecordStatusToTimeline -date $LogEntryList[$i].DateTime -status "Info" -detail "######################## Device Preparation mode (APv2) detected ########################" -logEntry "Line $($LogEntryList[$i].Line)" -Color 'Yellow'
		...
		# Out from Device Preparation mode detected
		if(($DetectedInAPv2Mode -eq $True) -and ($DetectedOutFromAPv2Mode -eq $False)) {
			if($LogEntryList[$i].Message -eq '[APv2] Successfully disabled APv2 mode.') {
				
				$DetectedOutFromAPv2Mode = $True
				
				$APv2ModeEndTime = $LogEntryList[$i].DateTime
				
				# Get Deployment time in minutes for event message
				$APv2DeploymentTimeMinutes = $null
				$APv2DeploymentTimeSeconds = $null
				
				if($APv2ModeStartTime -and $APv2ModeEndTime) {
					$APv2DeploymentDurationDateTimeObject = New-Timespan (Get-Date $APv2ModeStartTime) (Get-Date $APv2ModeEndTime)

					# Get totalminutes
					$APv2DeploymentTimeTotalMinutes = $APv2DeploymentDurationDateTimeObject.TotalMinutes
					$APv2DeploymentTimeTotalMinutes = [math]::Round($APv2DeploymentTimeTotalMinutes,0)
					
					# Get seconds
					$APv2DeploymentTimeSeconds = $APv2DeploymentDurationDateTimeObject.Seconds
				}

				$DeploymentTimeDetail = $null
				if($APv2DeploymentTimeTotalMinutes -and $APv2DeploymentTimeSeconds) {
					$DeploymentTimeDetail = "Deployment time $APv2DeploymentTimeTotalMinutes Minutes $APv2DeploymentTimeSeconds Seconds"
				}
			
				$LogEntryList[$i].ProcessRunTime = "$Detail $DeploymentTimeDetail"
			
				RecordStatusToTimeline -date $LogEntryList[$i].DateTime -status "Info" -detail "######################## Device Preparation mode (APv2) disabled. ########################" -logEntry "Line $($LogEntryList[$i].Line)" -Color 'Yellow'
				
				if($DeploymentTimeDetail) {
					RecordStatusToTimeline -date $LogEntryList[$i].DateTime -status "Info" -detail "######################## $DeploymentTimeDetail ########################" -logEntry "Line $($LogEntryList[$i].Line)" -Color 'Yellow'
				}
		...
		if($LogEntryList[$i].Message -like '`[PowerShell`] Processing policy with id = *') {
			if($LogEntryList[$i].Message -Match '^\[PowerShell\] Processing policy with id = (.*) for user (.*)$') {
				...
				$IntuneScriptContentInClearText = $null
				$DetailToolTip = $null
				...
				# Get Powershell script real name if parameter -Online was specified
				# and we have downloaded Powershell script information
				if($IdHashtable.ContainsKey($PowershellScriptPolicyIdStart)) {
					# Powershell script information was downloaded earlier
					$PowershellScriptPolicyNameStart = $IdHashtable[$PowershellScriptPolicyIdStart].name

				} else {
					# We set Powershell script name to be the Id which we found from log file
					$PowershellScriptPolicyNameStart = $PowershellScriptPolicyIdStart
				}

				# Get Powershell script in clear text if possible
				$IntuneScriptContentInClearText = Get-IntunePowershellScriptContentInCleartext $PowershellScriptPolicyIdStart
				$detailToolTip = "##########################################`n# Script:`n`n$IntuneScriptContentInClearText"
				
				if($ShowAllTimelineEvents) {

					if($DetailToolTip) {
						# We have $DetailToolTip which includes the Powershell script in clear text
						
						RecordStatusToTimeline -date $LogEntryList[$i].DateTime -status "Start" -Type "Powershell script" -Intent "Execute" -detail "`t$PowershellScriptPolicyNameStart for user $PowershellScriptForUserNameStart" -detailToolTip $DetailToolTip -logEntry "Line $($LogEntryList[$i].Line)"
					} else {
						# We do NOT have $DetailToolTip which includes the Powershell script in clear text
		...
		if($LogEntryList[$i].Message -like '`[PowerShell`] User Id = *') {
			if($LogEntryList[$i].Message -Match '^\[PowerShell\] User Id = (.*), Policy id = (.*), policy result = (.*)$') {
				...
				$FoundPowershellScriptEndMessageIndex = $i
				
				$b = $i - 1

				# Go up and try to find script output message
				# Only go here if we found Powershell script start message earlier
				# and only if Powershell start PolicyId equals Powershell end PolicyId
				
				# DEBUG
				#Write-Host "b: $b, PowershellScriptStartLogEntryIndex: $PowershellScriptStartLogEntryIndex, StartLogEntryFound: $PowershellScriptStartLogEntryFound, PolicyIdStart: $PowershellScriptPolicyIdStart, PolicyIdEnd: $PowershellScriptPolicyIdEndProcess"
				#Write-Host ""
				
				# This works, somewhat at least
				# It may not find all outputs because sometimes AgentExecutor is out of sync with Intune Log
				# But this is acceptable solution for now
				# Make it better later :)
				While (($b -ge 0) -and ($b -gt $PowershellScriptStartLogEntryIndex) -and ($PowershellScriptStartLogEntryFound) -and ($PowershellScriptPolicyIdStart -eq $PowershellScriptPolicyIdEndProcess)) {

					# Save output message to variable $scriptOutputStringArray
					if(($LogEntryList[$b].Message -Match '^write output done. (output =.*)$') -and ($LogEntryList[$b].Component -eq 'AgentExecutor')) {
						...
						While(($LogEntryList[$c].Multiline -ne '---->') -and ($LogEntryList[$c].Component -eq 'AgentExecutor') -and ($c -lt $b)) {
							$scriptOutputStringArray += $LogEntryList[$c].Message
				...
				if($IdHashtable.ContainsKey($PowershellScriptPolicyIdEndProcess)) {
					...
					# Get Powershell in cleartext if property exists
					# Property should exist if starting event was found earlier and -Online parameter is specified
					if($IdHashtable[$PowershellScriptPolicyIdEndProcess].scriptContentClearText) {
						# Property scriptContentClearText exists
						$IntuneScriptContentInClearText = $IdHashtable[$PowershellScriptPolicyIdEndProcess].scriptContentClearText
					} else {
						# Property scriptContentClearText does NOT exist
					}
				} else {
				...
				# Get Powershell script in clear text if possible
				$IntuneScriptContentInClearText = Get-IntunePowershellScriptContentInCleartext $PowershellScriptPolicyIdEndProcess
				$DetailToolTip = $IntuneScriptContentInClearText
				...
				if($scriptOutputStringArray) {
					$detailToolTip = "##########################################`n# Output:`n`n"
					$detailToolTip += $scriptOutputStringArray | Out-String
					$detailToolTip += "`n`n##########################################`n# Script:`n`n$IntuneScriptContentInClearText"
				} else {
					$detailToolTip = "##########################################`n# Output:`n`n"
				...
				if($PowershellScriptPolicyResultEndProcess -eq 'Success') {
					...
					$Detail = "$PowershellScriptPolicyNameEnd for user $PowershellScriptForUserNameEndProcess"
					#$DetailToolTip = "$IntuneScriptContentInClearText"
					
					RecordStatusToTimeline -date $LogEntryList[$i].DateTime -status "Success" -Type "Powershell script" -Intent "Execute" -detail $Detail -DetailToolTip $DetailToolTip -Seconds $PowershellScriptRunTimeTotalSeconds -logEntry "Line $($LogEntryList[$i].Line)" -Color 'Green'
					...
					RecordStatusToTimeline -date $LogEntryList[$i].DateTime -status "Failed" -Type "Powershell script" -Intent "Execute" -detail $Detail -DetailToolTip $DetailToolTip -Seconds $PowershellScriptRunTimeTotalSeconds -logEntry "Line $($LogEntryList[$i].Line)" -Color 'Red'
					...
					$Detail = "Powershell script $PowershellScriptPolicyNameEnd for user $PowershellScriptForUserNameEndProcess ($PowershellScriptRunTimeTotalSeconds seconds)"
					$DetailToolTip = "$IntuneScriptContentInClearText"
					
					RecordStatusToTimeline -date $LogEntryList[$i].DateTime -status "Script end" -Type "Powershell script" -Intent "Execute" -detail $Detail -DetailToolTip $DetailToolTip -logEntry "Line $($LogEntryList[$i].Line)" -Color 'Yellow'
				}
					
				$PowershellScriptStartLogEntryIndex=$null
				$PowershellScriptStartLogEntryFound = $false
				
				$FoundPowershellScriptEndMessageIndex = $null
				$PowershellScriptEndLogEntryFound = $false
		...
		if($LogEntryList[$i].Message -like '"C:\Program Files (x86)\Microsoft Intune Management Extension\agentexecutor.exe"  -remediationScript  ""C:\Windows\IMECache\HealthScripts\*\*.ps1""*') {
			...
			if($LogEntryList[$i].Message -Match '^"C:\\Program Files \(x86\)\\Microsoft Intune Management Extension\\agentexecutor.exe"  -remediationScript  ""C:\\Windows\\IMECache\\HealthScripts\\(.*)_[0-9]+\\(.*)\.ps1"".*$') {
				...
				$ScriptInClearText = $null
				$intent = $null

				# Get Detection or Remediate action
				...
				if($RemediationScriptPolicyType -eq 6) {
					...
					# Get Detect script in cleartext
					if($intent -eq 'Detect') {
						$ScriptInClearText = Get-IntuneRemediationDetectionScriptContentInCleartext $PolicyId
					}
					
					# Get Remediate script in cleartext
					if($intent -eq 'Remediate') {
						$ScriptInClearText = Get-IntuneRemediationRemediateScriptContentInCleartext $PolicyId
					}
				} elseif ($RemediationScriptPolicyType -eq 8) {
					$RemediationScriptPolicyTypeName = 'Custom Compliance'
					$ScriptInClearText = Get-IntuneCustomComplianceScriptContentInCleartext $PolicyId
				} else {
				...
				for($b = $i + 1; $b -lt $LogEntryList.Count; $b++ ) {
					...
					if(($LogEntryList[$b].Message -Match '^Powershell execution is done, exitCode = (.*)$') -and ($LogEntryList[$b].Thread -eq $Thread)) {
						...
						if($scriptOutputStringArray) {
							$detailToolTip = "##########################################`n# Output:`n`n"
							$detailToolTip += $scriptOutputStringArray | Out-String
							$detailToolTip += "`n`n##########################################`n# Script:`n`n$ScriptInClearText"
						} else {
							$detailToolTip = "##########################################`n# Output:`n`n"
							$detailToolTip += "Runtime information not available"
							$detailToolTip += "`n`n##########################################`n# Script:`n`n$ScriptInClearText"
						...
						if($RemediationExitCode -eq 0) {
							...
							if($RemediationScriptPolicyTypeName -eq 'remediate') {
								...
								# This should be Detection with Not Detected status
								$status = 'Not Detected'
								$color = 'Yellow'
							}
					...
					if($LogEntryList[$b].Message -like '"C:\Program Files (x86)\Microsoft Intune Management Extension\agentexecutor.exe"  -remediationScript  ""C:\Windows\IMECache\HealthScripts\*\*.ps1""*') {
						...
						$status = 'Start'
						$detail = "$RemediationScriptName"
						$detailToolTip = "##########################################`nScript:`n`n$ScriptInClearText"
...
#>			
	...
	<style>
	  ...
	  /* ################################################### */
	  /* ToolTip related style. Do not remove   */
	  ...
	  .tooltip .tooltiptext {
		...
		min-width: 600px; /* Set a minimum width for the tooltip */
		max-width: 90vw; /* Set a maximum width to 90% of the viewport width */
		background-color: #f9f9f9; /* White background for the tooltip */
		...
		opacity: 0;
		transition: opacity 0.3s;
		padding: 15px 15px 15px 15px; /* Set bottom padding to 0 */
		font-family: Consolas, "Courier New", Monaco, monospace; /* Monospaced font for code */
		font-weight: normal; /* Ensure text is not bold */
		white-space: pre; /* Prevent wrapping and preserve all spaces and line breaks */
...
"@
		...
		//});		
			...
			// Add right-click menu for copying tooltip text
			function addRightClickCopyTooltip(tableId, tooltipSourceColumnName) {
			  let table = document.getElementById(tableId);
			  ...
			  contextMenu.innerHTML = '<ul><li id="copyTooltip">Copy ToolTip Text</li></ul>';
			  ...
			  function showContextMenu(event, tooltipText) {
				...
				// Copy tooltip text when clicking "Copy Tooltip"
				document.getElementById('copyTooltip').onclick = function () {
				  navigator.clipboard.writeText(tooltipText).then(() => {
					alert('Tooltip text copied to clipboard!');
				  }).catch(err => {
			  ...
			  for (let row of table.rows) {
				if (row.cells[sourceColumnIndex] && row.cells[sourceColumnIndex].innerText.trim() !== "") {
				  ...
				  row.addEventListener('contextmenu', function (event) {
					showContextMenu(event, tooltipText);
				  });
				}
			  }
		...
		window.onload = function() {
			...
			// Add right-click "Copy Tooltip" functionality
			addRightClickCopyTooltip(HTML_TABLE_ID, 'DetailToolTip');
