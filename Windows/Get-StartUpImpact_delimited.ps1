# Get Intune failed app deployments - Structured Output Version

try {
    # Get failed app deployments from Intune logs
    ...
    if (Test-Path $IntuneLogPath) {
        # Parse log for failed deployments
        $LogContent = Get-Content $IntuneLogPath -Tail 1000
        $FailedDeployments = @{}
        $AppNames = @{}
        
        foreach ($Line in $LogContent) {
            if ($Line -match "App installation failed.*AppId:\s*([a-f0-9\-]+).*AppName:\s*([^,]+)" -or
                $Line -match "Installation failed.*([a-f0-9\-]+).*DisplayName.*?([^,\]]+)") {
                
                $AppId = $matches[1]
                $AppName = $matches[2].Trim()
                
                if ($FailedDeployments.ContainsKey($AppId)) {
                    $FailedDeployments[$AppId]++
                } else {
                    $FailedDeployments[$AppId] = 1
        ...
        $TopFailed = $FailedDeployments.GetEnumerator() | 
        ...
        foreach ($App in $TopFailed) {
            $AppId = $App.Key
            $Count = $App.Value
            $Name = if ($AppNames[$AppId]) { $AppNames[$AppId] } else { "Unknown" }
            
            $TopFailedApps += $AppId
            $AppFailureCounts += $Count.ToString()
            $AppNamesList += $Name
        }
        
        # Get last reboot time
        $LastReboot = (Get-CimInstance -ClassName Win32_OperatingSystem).LastBootUpTime
        $RebootFormatted = $LastReboot.ToString("yyyy-MM-dd HH:mm:ss")
        
        # Output structured data
        Write-Output "TopFailedApps:$($TopFailedApps -join '|')"
        Write-Output "AppFailureCounts:$($AppFailureCounts -join '|')"
        Write-Output "AppNames:$($AppNamesList -join '|')"
        Write-Output "LastRebootTime:$RebootFormatted"
        Write-Output "TotalFailedApps:$($FailedDeployments.Count)"
        
    } else {
        Write-Output "TopFailedApps:No Intune log found"
        Write-Output "AppFailureCounts:0"
        Write-Output "AppNames:N/A"
        Write-Output "LastRebootTime:Unknown"
        Write-Output "TotalFailedApps:0"
    }
    
} catch {
    Write-Output "TopFailedApps:Error occurred"
    Write-Output "AppFailureCounts:0" 
    Write-Output "AppNames:$($_.Exception.Message)"
    Write-Output "LastRebootTime:Unknown"
    Write-Output "TotalFailedApps:0"
}
